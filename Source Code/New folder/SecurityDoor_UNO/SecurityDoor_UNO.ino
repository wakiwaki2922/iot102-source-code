...
SoftwareSerial nodeMCU(Rx, Tx); //Khởi tạo cổng serial mềm
...
void checkKeys()
{
  ...
  if (nodeMCU.available() > 0) {
    String command = nodeMCU.readStringUntil('\n');
    if (command == "openDoor") {
      Serial.println("Received command to open door from NodeMCU");
      delay(1000);
      tgDoor();
    }
  }
